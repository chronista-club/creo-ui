...
const DECISIONS = [
  {
    id: 'D-1',
    topic: 'Editor の粒度',
    decision: 'Mode (universal state) — instance 命名は使わない',
  },
  {
    id: 'D-2',
  ...
  },
  {
    id: 'D-5',
    topic: 'Field source',
    decision: 'あらかじめ (framework) + カスタム (app-specific) の 2 ルート',
  },
  {
    id: 'D-6',
    topic: '非侵襲性',
    decision: 'Editor Layer は Content の座標・可視性・操作を奪わない',
  },
  {
    id: 'D-7',
    topic: 'Mode toggle',
  ...
  {
    id: 'D-10',
    topic: 'AI agent access',
    decision: '同 protocol を MCP 経由で (enter/select/set/subscribe/exit)',
  },
  {
    id: 'D-11',
  ...
  },
  {
    ...
    topic: '段階',
    decision:
      'Phase 1 = 設計 memo + tokens / Phase 2 = Web 実装 + MCP / Phase 3+ = Swift / theme 切替',
  },
] as const
    ...
    <>
      <header class="docs-page-header">
        ...
        <p class="docs-page-lead">
          ...
          <code>@chronista-club/creo-ui-editor-host</code> for Web、 将来 Swift / Rust)。
        </p>
      </header>
      ...
      <section>
        ...
        <ul class="docs-bullet-list">
          <li>
            Content layer の上に <strong>4 方向 overlay</strong> として展開 — 別 page に行かない
          </li>
          <li>
            field 宣言と layout を <strong>schema として規定</strong> — 全 app で同じ操作感
          </li>
      ...
      <Decision number="D-1" title="Editor の粒度 — Mode であって Instance ではない">
        <h3>Why</h3>
        <p>
          「Studio」 「DevEditor」 等の <strong>instance 命名</strong> は app ごとに分裂を招く。
          Studio の Editor と DevEditor の Editor が「似てるけど違う」 と分岐すると、
          共通体験が崩壊する。 また AI agent 視点でも instance ごとに違う protocol
          を学習する負債が出る。
        </p>
      ...
      <Decision number="D-6" title="非侵襲性 — Content を一切触らない">
        <h3>Why</h3>
        <p>
          Editor Layer が Content の座標を変えると 「Editor ON にしたら layout が崩れた」
          という最悪の体験。 dev tools が「触ると壊れる」 場所になり、 開発者 / designer が Editor
          Mode を恐れる。 結果、 機能が使われない。
        </p>
        <h3>How it shows up</h3>
        <p>
      ...
      <Decision number="D-7" title="Mode toggle — 手動のみ、 自動 ON なし">
        <h3>Why</h3>
        <p>
          自動 ON (例: dev 環境で常時 ON) は 「いつのまにか UI が変わってる」 という驚きを生む。 AI
          agent の操作で勝手に ON になるのも危険 — 編集してるつもりの無い user の content を
          意図せず変える事故が起きる。
        </p>
        <h3>How it shows up</h3>
        <p>
          ON 経路は明示的 4 つのみ: keyboard shortcut (<code>Ctrl+Shift+E</code>)、 floating
          button、 programmatic <code>host.mode.enable()</code>、 MCP <code>editor_mode_enter</code>
          。 dev / production 区別なし、 user 意思のみで切替。
        </p>
      </Decision>
      ...
      <Decision number="D-11" title="Protocol owner — creo-ui が schema、 consumer が runtime">
        ...
        <p>
          ...
          Web runtime: <code>packages/editor-host/</code> (@chronista-club/creo-ui-editor-host
          npm)。 Swift / Rust runtime: 将来別 package。
        </p>
      </Decision>
      ...
      <section>
        <h2 class="docs-section-title">
          実装 — @chronista-club/creo-ui-editor-host (Web reference)
        </h2>
        <p class="docs-page-helper">
