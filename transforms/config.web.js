 ...
 * Consumed by creoui (packages/web).
 *
 * Theme matrix (0.1.0+):
 *   tokens/color/themes/*.json には `color.themes.{theme-id}.*` path の
 *   token を宣言する。custom format `css/creoui-themed` は、
 ...
 *   配布形が creoui Web の default。source tokens (tokens/<cat>/<file>.json)
