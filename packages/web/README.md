...
```ts
import 'creo-ui-web/tokens.css'      // CSS variable + 8 theme + reduced-motion guard
import 'creo-ui-web/components.css'  // 28 component (button / input / dialog / drawer 等)
...
## Token catalog — 5 tier convention (v0.17.0 から完全統一)

すべての dimension scale token は **`xs / s / m / l / xl` の 5 tier** に統一。 `sm/md/lg` Tailwind 流命名は **完全廃止** (廃止 history は [`docs/migration/v0.14-to-v0.17.md`](https://github.com/chronista-club/creo-ui/blob/main/docs/migration/v0.14-to-v0.17.md))。
...
```ts
import { ColorBrandPrimary, SpacingM, RadiusS } from 'creo-ui-web/tokens.js'
...
<button class="creo-btn" data-variant="ghost" data-size="sm">Cancel</button>
...
> **note**: `data-size="sm/md/lg"` 等 component 固有の attribute は **token と独立した axis** で、 v0.17 時点では Tailwind 流命名 (`sm` / `md` / `lg`) が残存。 token reality (5 tier `s/m/l`) との convention 統合は v0.18 で別途検討予定。
...
過去 release からの upgrade は [`docs/migration/v0.14-to-v0.17.md`](https://github.com/chronista-club/creo-ui/blob/main/docs/migration/v0.14-to-v0.17.md) を参照。 v0.17 以降は **5 tier convention 安定保証** (sm/md/lg 由来の rename はこれ以上発生しない設計)。
