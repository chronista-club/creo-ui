...
```bash
bun add creoui-editor-host creoui solid-js
```
...
`useEditorHost().mcp` に `listFields` / `getValue` / `setValue` / `mode` / `enable` / `disable` を expose。Phase 2b の `creoui-editor-host-mcp` (予定) がこれを stdio MCP server として公開、Claude 等から直接 field を操作できる。
...
```tsx
import { THEME_INFO, THEME_IDS, DEFAULT_THEME_ID, SWATCH_ROWS } from 'creoui-editor-host'
```
