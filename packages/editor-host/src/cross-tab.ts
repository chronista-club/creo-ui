/**
 * @chronista-club/creoui-editor-host — Cross-tab sync (F5)
 *
...
function defaultChannelName(namespace: string): string {
  return `@chronista-club/creoui-editor-host:${namespace}`
}
...
export function installCrossTabSync(host: EditorHost, opts: CrossTabOptions = {}): () => void {
  ...
  const namespace = opts.namespace ?? '@chronista-club/creoui-editor-host'
