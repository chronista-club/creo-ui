...
#### F-3 web 実装の階梯 (2026-05-04 articulate)

Web 単体でも 「metaphor → literal 3D」 の **段階** が存在する:

| 階梯 | 実装 | depth 表現 |
|---|---|---|
| **3a. CSS 3D** (現行) | `transform: translateZ(...)` + `perspective` | 視覚的奥行き (parallax)、 平面 device には eye-tricked depth |
| **3b. `<model>` element** (visionOS 26+) | HTML5 `<model stagemode="orbit"><source src="*.usdz"></model>` | **物理 3D entity**、 VP で stereoscopic display + pinch interaction、 子の `<img>` が他 browser での graceful fallback |
| **3c. WebXR immersive-ar session** | `navigator.xr.requestSession('immersive-ar')` | フル immersive、 ARKit 経由で hand pose / world tracking 直接 access (Phase B-spatial-web の 真の immersive content 用) |

`<model>` (3b) が **「protocol を変えずに representation だけ richer になる」** F-3 の真意を
最も直接表現する web 実装。 同じ HTML markup で:
- VP Safari (visionOS 26+) → 物理 3D entity として render、 user は pinch-and-drag で rotate
- 他 browser → 子の `<img>` が fallback として表示
...
| Platform | F-3 表現 | depth の literal さ |
|---|---|---|
| Web (CSS 3D) | `translateZ` + perspective | 視覚的のみ |
| **Web on visionOS Safari + `<model>`** | **`<model>` 物理 3D entity** | **literal stereoscopic 3D** ← Web 単体で literal 達成 |
| TUI (ratatui) | dim/bright | 知覚的 |
| SwiftUI native | Liquid Glass + shadow | 知覚的 |
| **visionOS native** (Phase B-spatial-native) | **RealityKit Entity の物理 z 座標** (head-relative meters) | **literal 物理空間 3D** |
