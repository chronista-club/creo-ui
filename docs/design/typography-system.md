...
> **暗黙のメッセージ**: 「Mizzolet を creoui の principal にしたい」 のような local font 採用は **creoui 側に追加しない**。consumer 側 (creo-web / fleetstage hq・backstage / apps/site 等) で **1 行 CSS override** すれば達成する。creoui は「sensible defaults + 明確な override API」を提供する design system であって、全 user の font 好みを内蔵する font museum ではない。
...
これだけで `app` family を参照する全 UI primitive (Principal Layout / CreoPageShell / CreoRail icon 等) が Mizzolet を使う。
...
```css
/* ✅ 推奨 (prepend pattern) — Mizzolet 不在環境では既存 chain に degrade */
:root {
  --typography-family-app: 'Mizzolet', var(--typography-family-app);
}

/* ❌ 非推奨 (replace pattern) — Mizzolet 不在環境で system default に落ち、Nerd Font icon が見えなくなる */
:root {
  --typography-family-app: 'Mizzolet';
}
```
...
| scope | selector 例 | 用途 |
|---|---|---|
| **global** | `:root { ... }` | app 全体で 1 font に統一 (mito の Mizzolet 採用は通常これ) |
...
@font-face {
  font-family: 'Mizzolet';
  src: url('/fonts/Mizzolet.woff2') format('woff2');
...
@font-face {
  font-family: 'Mizzolet';
  src: url('/fonts/Mizzolet-Bold.woff2') format('woff2');
  font-display: swap;
  font-weight: 700;
}
...
### 4.1. Mizzolet を principal font に (mito の典型 use case)
...
Mizzolet が install 済の Mac では Mizzolet で描画、未 install / 別 OS では既存 chain (JetBrainsMono Nerd Font Mono → PlemolJP → monospace) に degrade。
