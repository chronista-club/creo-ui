...
3 相は一方向ではない。`kyu`（検証 NG）→ `ha`（再編集）、`kyu` → `jo`（別対象）への戻りがある。この**相内ループ**が §5 の Driver loop と接続する。
...
enum EditOutcome {
    ...
    Reborn(EditContext)    // 編集継続 — 次の bracket cycle へ持ち越す（undo target を含む）
...
編集を**終端させず**、次の bracket cycle へ持ち越す。`EditContext` は次の `enter` の `Input` になりうるリッチな状態。

`Reborn` は creoui にとって 2 つの用途を持つ:

1. **AI 編集 loop の 1 周回** — AgentDriver が `set_value → 観察 → set_value` を繰り返すとき、各周回は `Reborn(EditContext)` で次へ繋がる（§5、nostos ADR-0003 option γ）。
2. **undo** — 過去の Active 状態へ巻き戻す。「1 つ前の field 値の集合」へ戻ることは、`Reborn` に**過去の `EditContext` を渡す**ことと同型。

> **読み替えで見えたこと（その 2）**: 用途 2 が成立するには `EditContext`（= ADT の `I`）が「次の入力」だけでなく **過去の Active 状態を復元できるリッチさ**を持つ必要がある。単なる「次に何を編集するか」のヒントでは undo target になれない。これは §9 で nostos ADR-0002 への feedback として articulate する。
...
### F-1 → ADR-0002（Outcome ADT）: `Reborn(I)` の `I` にリッチさが要る

ADR-0001 Axis B「3 variant で過不足ないか」への現場回答。

creoui は `Reborn` を **2 用途**で使う（§3）── (a) AI 編集 loop の次周回、(b) **undo（過去 Active 状態への巻き戻し）**。用途 (b) が成立するには、`Reborn(I)` の `I`（creoui では `EditContext`）が「次の入力ヒント」ではなく **過去の Active 状態を復元できるリッチな snapshot** でなければならない。

- **提案**: ADR-0002 で `Outcome` の `I` 型に対し「`I` は `enter` の `Input` として再投入可能、かつ過去 Active の復元に足る情報量を持つ」という**契約**を明文化する。`I` を単なる「次回パラメータ」と狭く規定しないこと。
