...
| attr | 値 | default | 意味 |
|---|---|---|---|
| `data-size` | `s` / `m` / `l` | `m` | dialog width |
...
| slot | token |
|---|---|
| backdrop | `color.surface.scrim-modal` (50% black、`::backdrop`) |
| dialog bg | `color.surface.surface` |
| border | `color.surface.border` 1px |
| border-radius | `radius.l` (22px、少し大きめで柔らかい存在感) |
| shadow | `shadow.l` (elevated 強調) |
| max-width (sm) | `320px` |
| max-width (md) | `480px` |
| max-width (lg) | `720px` |
...
```html
<dialog class="creo-dialog" data-size="m" id="confirm-dlg">
